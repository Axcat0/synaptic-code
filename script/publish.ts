...
const PACKAGE_NAME = "synaptic-code"
...
console.log("=== Publishing synaptic-code (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `synaptic-code-${platform}`
...
async function getContributors(previous: string): Promise<string[]> {
  ...
  try {
    const compare =
      await $`gh api "/repos//synaptic-code/compare/v${previous}...HEAD" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/synaptic-code.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
