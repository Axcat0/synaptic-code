 ...
 * @returns {string} Package name like "synaptic-code-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `synaptic-code-${os}-${arch}${suffix}`;
}
...
function getBaselinePlatformPackage({ platform, arch, libcFamily }) {
  ...
  if (platform === "darwin") {
    return "synaptic-code-darwin-x64-baseline";
  }

  if (platform === "win32") {
    return "synaptic-code-windows-x64-baseline";
  }
  ...
  if (platform === "linux") {
    ...
    if (libcFamily === "musl") {
      return "synaptic-code-linux-x64-musl-baseline";
    }

    return "synaptic-code-linux-x64-baseline";
  }
 ...
 * @returns {string} Relative path like "synaptic-code-darwin-arm64/bin/synaptic-code"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/synaptic-code${ext}`;
}
