---
description: Publish synaptic-code to npm via GitHub Actions workflow
argument-hint: <patch|minor|major>
---

<command-instruction>
You are the release manager for synaptic-code. Execute the FULL publish workflow from start to finish.
...
```bash
npm view synaptic-code version
```
...
for PLATFORM in $PLATFORMS; do
  npm view "synaptic-code-${PLATFORM}" version
done
```
...
| Package | Description |
|---------|-------------|
| `synaptic-code-darwin-arm64` | macOS Apple Silicon |
| `synaptic-code-darwin-x64` | macOS Intel |
| `synaptic-code-linux-x64` | Linux x64 (glibc) |
| `synaptic-code-linux-arm64` | Linux ARM64 (glibc) |
| `synaptic-code-linux-x64-musl` | Linux x64 (musl/Alpine) |
| `synaptic-code-linux-arm64-musl` | Linux ARM64 (musl/Alpine) |
| `synaptic-code-windows-x64` | Windows x64 |
...
- New version number
- GitHub release URL: https://github.com//synaptic-code/releases/tag/v{version}
- npm package URL: https://www.npmjs.com/package/synaptic-code
...
<current-context>
<published-version>
!`npm view synaptic-code version 2>/dev/null || echo "not published"`
</published-version>
<local-version>
...
<recent-commits>
!`npm view synaptic-code version 2>/dev/null | xargs -I{} git log "v{}"..HEAD --oneline 2>/dev/null | head -15 || echo "no commits"`
</recent-commits>
</current-context>
