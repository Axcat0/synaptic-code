...
import type { RuntimeFallbackConfig, SynapticCodeConfig } from "../../config"
...
describe("runtime-fallback", () => {
  ...
  function createMockPluginConfigWithCategoryFallback(fallbackModels: string[]): SynapticCodeConfig {
    return {
      categories: {
        test: {
  ...
  describe("fallback models configuration", () => {
    function createMockPluginConfigWithAgentFallback(agentName: string, fallbackModels: string[]): SynapticCodeConfig {
      return {
        agents: {
          [agentName]: {
            fallback_models: fallbackModels,
