...
describe("findPluginEntry", () => {
  ...
  test("returns unpinned for bare package name", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["synaptic-code"] }))
  ...
  test("returns unpinned for latest dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["synaptic-code@latest"] }))
  ...
  test("returns unpinned for beta dist-tag", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["synaptic-code@beta"] }))
  ...
  test("returns pinned for explicit semver", () => {
    ...
    fs.writeFileSync(configPath, JSON.stringify({ plugin: ["synaptic-code@3.5.2"] }))
