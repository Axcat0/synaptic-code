...
describe("pinned-version-updater", () => {
  ...
  describe("updatePinnedVersion", () => {
    test("updates pinned version in config", () => {
      ...
      const config = JSON.stringify({
        plugin: ["synaptic-code@3.1.8"],
      })
      fs.writeFileSync(configPath, config)

      //#when
      const result = updatePinnedVersion(configPath, "synaptic-code@3.1.8", "3.4.0")

      //#then
      expect(result).toBe(true)
      const updated = fs.readFileSync(configPath, "utf-8")
      expect(updated).toContain("synaptic-code@3.4.0")
      expect(updated).not.toContain("synaptic-code@3.1.8")
    })
  ...
  describe("revertPinnedVersion", () => {
    test("reverts from failed version back to original entry", () => {
      //#given
      const config = JSON.stringify({
        plugin: ["synaptic-code@3.4.0"],
      })
      fs.writeFileSync(configPath, config)

      //#when
      const result = revertPinnedVersion(configPath, "3.4.0", "synaptic-code@3.1.8")

      //#then
      expect(result).toBe(true)
      const reverted = fs.readFileSync(configPath, "utf-8")
      expect(reverted).toContain("synaptic-code@3.1.8")
      expect(reverted).not.toContain("synaptic-code@3.4.0")
    })
    ...
    test("reverts to unpinned entry", () => {
      ...
      //#when
      const result = revertPinnedVersion(configPath, "3.4.0", "synaptic-code")

      //#then
      expect(result).toBe(true)
      const reverted = fs.readFileSync(configPath, "utf-8")
      expect(reverted).toContain('"synaptic-code"')
      expect(reverted).not.toContain("synaptic-code@3.4.0")
    })
  ...
  describe("update then revert roundtrip", () => {
    test("config returns to original state after update + revert", () => {
      ...
      //#when
      updatePinnedVersion(configPath, "synaptic-code@3.1.8", "3.4.0")
      revertPinnedVersion(configPath, "3.4.0", "synaptic-code@3.1.8")

      //#then
      const finalConfig = fs.readFileSync(configPath, "utf-8")
      expect(finalConfig).toContain("synaptic-code@3.1.8")
      expect(finalConfig).not.toContain("synaptic-code@3.4.0")
    })
  })
})
