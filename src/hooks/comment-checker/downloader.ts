...
const REPO = "/go-claude-code-comment-checker"
...
/**
 * Get the cache directory for synaptic-code binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "synaptic-code", "bin")
  }
...
/**
 * Get the version from the installed @/comment-checker package.
 */
function getPackageVersion(): string {
  try {
    const require = createRequire(import.meta.url)
    const pkg = require("@/comment-checker/package.json")
    return pkg.version
  } catch {
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[synaptic-code] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[synaptic-code] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[synaptic-code] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[synaptic-code] Comment checking disabled.`)
    return null
  }
}
