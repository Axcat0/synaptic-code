...
export async function runCliInstaller(args: InstallArgs, version: string): Promise<number> {
  ...
  if (!validation.valid) {
    ...
    console.log()
    printInfo(
      "Usage: bunx synaptic-code install --no-tui --claude=<no|yes|max20> --gemini=<no|yes> --copilot=<no|yes>",
    )
    console.log()
    return 1
  }
  ...
  printStep(step++, totalSteps, "Adding synaptic-code plugin...")
  ...
  printStep(step++, totalSteps, "Writing synaptic-code configuration...")
