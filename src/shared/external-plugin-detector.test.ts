...
describe("external-plugin-detector", () => {
  ...
  describe("detectExternalNotificationPlugin", () => {
    ...
    test("should return detected=false when only synaptic-code is configured", () => {
      // given - opencode.json with only synaptic-code
      const opencodeDir = path.join(tempDir, ".opencode")
      fs.mkdirSync(opencodeDir, { recursive: true })
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["synaptic-code"] })
      )
      ...
      expect(result.detected).toBe(false)
      expect(result.pluginName).toBeNull()
      expect(result.allPlugins).toContain("synaptic-code")
    })
    ...
    test("should detect opencode-notifier plugin", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["synaptic-code", "opencode-notifier"] })
      )
    ...
    test("should detect opencode-notifier with version suffix", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["synaptic-code", "opencode-notifier@1.2.3"] })
      )
    ...
    test("should detect @mohak34/opencode-notifier", () => {
      ...
      fs.writeFileSync(
        path.join(opencodeDir, "opencode.json"),
        JSON.stringify({ plugin: ["synaptic-code", "@mohak34/opencode-notifier"] })
      )
