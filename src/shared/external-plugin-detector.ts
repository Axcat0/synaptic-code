/**
 * Detects external plugins that may conflict with synaptic-code features.
...
/**
 * Known notification plugins that conflict with synaptic-code's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[synaptic-code] External notification plugin detected: ${pluginName}

Both synaptic-code and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   synaptic-code's session-notification has been auto-disabled.

   To use synaptic-code's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in synaptic-code.json`
}
