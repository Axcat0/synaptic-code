...
24 schema files composing `SynapticCodeConfigSchema`. Zod v4 validation with `safeParse()`. All fields optional — omitted fields use plugin defaults.

## SCHEMA TREE

```
config/schema/
├── synaptic-code-config.ts    # ROOT: SynapticCodeConfigSchema (composes all below)
...
2. Add field to `synaptic-code-config.ts` root schema
