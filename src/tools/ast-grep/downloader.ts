...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[synaptic-code] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[synaptic-code] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[synaptic-code] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[synaptic-code] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
