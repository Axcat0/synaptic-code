...
export function injectGitMasterConfig(template: string, config?: GitMasterConfig): string {
	...
	if (commitFooter) {
		const footerText =
			typeof commitFooter === "string"
				? commitFooter
				: "Ultraworked with [Sisyphus](https://github.com//synaptic-code)"
	...
	if (includeCoAuthoredBy) {
		...
		sections.push("```")
		sections.push("Co-authored-by: Sisyphus <clio-agent@>")
		sections.push("```")
		sections.push("")
	}
	...
	if (commitFooter && includeCoAuthoredBy) {
		...
		sections.push("```bash")
		sections.push(
			`git commit -m "{Commit Message}" -m "${footerText}" -m "Co-authored-by: Sisyphus <clio-agent@>"`
		)
		sections.push("```")
		...
		sections.push("```bash")
		sections.push(
			"git commit -m \"{Commit Message}\" -m \"Co-authored-by: Sisyphus <clio-agent@>\""
		)
		sections.push("```")
	}
