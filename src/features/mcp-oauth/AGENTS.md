...
18 files. Full OAuth 2.0 authorization flow for MCP servers requiring authentication. Implements PKCE (RFC 7636), Dynamic Client Registration (DCR, RFC 7591), and resource indicators (RFC 8707). Used by `bunx synaptic-code mcp-oauth login`.
...
```bash
bunx synaptic-code mcp-oauth login <server-url>   # Full PKCE flow
bunx synaptic-code mcp-oauth logout <server-url>  # Revoke + delete token
bunx synaptic-code mcp-oauth status               # List stored tokens
```
