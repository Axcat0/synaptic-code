...
Complete reference for `synaptic-code.jsonc` configuration. This document covers every available option with examples.
...
1. `.opencode/synaptic-code.jsonc` / `.opencode/synaptic-code.json`
2. User config (`.jsonc` preferred over `.json`):

| Platform | Path |
|----------|------|
| macOS/Linux | `~/.config/opencode/synaptic-code.jsonc` |
| Windows | `%APPDATA%\opencode\synaptic-code.jsonc` |

JSONC supports `// line comments`, `/* block comments */`, and trailing commas.

Enable schema autocomplete:
```json
{ "$schema": "https://raw.githubusercontent.com//synaptic-code/dev/assets/synaptic-code.schema.json" }
```

Run `bunx synaptic-code install` for guided setup. Run `opencode models` to list available models.
...
Run `bunx synaptic-code doctor --verbose` to see effective model resolution for your config.
