...
Complete reference for the `synaptic-code` command-line interface.
...
Interactive installation tool for initial Synaptic-Code setup. Provides a TUI based on `@clack/prompts`.
...
3. **Configuration File Creation**: Generates `opencode.json` or `synaptic-code.json` files
4. **Plugin Registration**: Automatically registers the synaptic-code plugin in OpenCode settings
...
Diagnoses your environment to ensure Synaptic-Code is functioning correctly. Performs 17+ health checks.

### Usage

```bash
bunx synaptic-code doctor
```
...
bunx synaptic-code mcp oauth login <server-name> --server-url https://api.example.com

# Login with explicit client ID and scopes
bunx synaptic-code mcp oauth login my-api --server-url https://api.example.com --client-id my-client --scopes "read,write"

# Remove stored OAuth tokens
bunx synaptic-code mcp oauth logout <server-name>

# Check OAuth token status
bunx synaptic-code mcp oauth status [server-name]
```
...
```bash
# Login
bunx synaptic-code auth login

# Logout
bunx synaptic-code auth logout

# Check current status
bunx synaptic-code auth status
```
...
1. **Project Level**: `.opencode/synaptic-code.json`
2. **User Level**: `~/.config/opencode/synaptic-code.json`
...
bunx synaptic-code doctor --verbose

# Check specific category only
bunx synaptic-code doctor --category authentication
```
...
bunx synaptic-code doctor --no-tui --json

# Save results to file
bunx synaptic-code doctor --json > doctor-report.json
```
